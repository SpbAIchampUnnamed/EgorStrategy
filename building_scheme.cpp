                ...
                if constexpr(add)
                {
                    for (auto[v, p]: from_mapping) {
                        for (auto x: g.edges[v]) {
                            ...
                            if (!e.flow)
                                continue;
                            if (auto it = ranges::lower_bound(to_mapping, pair(e.to, 0)); it != to_mapping.end() &&
                                                                                          it->first == e.to) {
            ...
            for (auto[p, type]: distribution) {
                ...
                if (bp.produceResource) {
                    for (auto[t, type]: distribution) {
                        if (!game.buildingProperties.at(type).workResources.contains(*bp.produceResource))
                            continue;
                        coefs[ranges::lower_bound(trans_vars, TransVar{p, t, bp.produceResource}) -
                              trans_vars.begin()] = 1;
                    }
                }
                for (auto[f, ft]: distribution) {
                    ...
                    if (bp.produceResource &&
                        game.buildingProperties.at(type).workResources.contains(*bp.produceResource))
                        coefs[ranges::lower_bound(trans_vars, TransVar{f, p, bp.produceResource}) -
                              trans_vars.begin()] = -1;
